...
{
	internal class MSBraceMatchingTagger
		: ITagger<TextMarkerTag>
	{
		...
		internal MSBraceMatchingTagger(ITextView view, ITextBuffer sourceBuffer)
		{
		...
		public IEnumerable<ITagSpan<TextMarkerTag>> GetTags(NormalizedSnapshotSpanCollection spans)
		{
			...
			if (m_braceList.ContainsKey(currentText))   //the key is the open brace
			{
				...
				if (MSBraceMatchingTagger.FindMatchingCloseChar(currentChar, currentText, closeChar, View.TextViewLines.Count, out pairSpan) == true)
				{
					
					yield return new TagSpan<TextMarkerTag>(new SnapshotSpan(currentChar, 1), new TextMarkerTag("brace matching"));
					yield return new TagSpan<TextMarkerTag>(pairSpan, new TextMarkerTag("brace matching"));
				}
			}
			else if (m_braceList.ContainsValue(lastText))    //the value is the close brace, which is the *previous* character 
			{
				var open = from n in m_braceList
						   ...
						   select n.Key;
				if (MSBraceMatchingTagger.FindMatchingOpenChar(lastChar, (char)open.ElementAt<char>(0), lastText, View.TextViewLines.Count, out pairSpan) == true)
				{
					yield return new TagSpan<TextMarkerTag>(new SnapshotSpan(lastChar, 1), new TextMarkerTag("brace matching"));
					yield return new TagSpan<TextMarkerTag>(pairSpan, new TextMarkerTag("brace matching"));
				}
			}
		}
